...
const AdminPagination = ({ currentPage, totalPages, onPageChange }) => {
  ...
  return (
    <nav className="flex flex-wrap items-center justify-center gap-2 py-2" aria-label="Pagination">
      <button
        type="button"
      ...
      {getPageNumbers().map((page, index) =>
        page === '...' ? (
          <span
            key={`ellipsis-${index}`}
            className="flex h-10 items-center px-2 text-surface-400 dark:text-surface-500"
          >
            …
          </span>
        ) : (
